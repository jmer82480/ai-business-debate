"""Devil's Advocate role — contrarian, stress-tester."""
...
from debate.prompts.phase4 import get_da_stress_test_prompt, get_stress_test_tool_schema
...
class DevilsAdvocate(BaseRole):
    ...
    def stress_test(self, idea_name: str, idea_id: str, deep_dive_text: str) -> tuple[dict[str, Any], str]:
        """Phase 4: final stress test on a finalist."""
        tool = get_stress_test_tool_schema()
        response = self._call_llm(
            get_da_stress_test_prompt(idea_name, deep_dive_text),
            tools=[tool],
            tool_choice={"type": "tool", "name": "submit_stress_test"},
        )
        data = self._extract_tool_data(response, "submit_stress_test")
